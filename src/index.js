...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (!user.pro && (user.todos.length >= 10)) {
    return response.status(403).json({ error: 'User is not allowed to create more todo. Upgrade to a Pro account.' });
  }
...
function checksTodoExists(request, response, next) {
  ...
  if (!todo) {
    return response.status(404).json({ error: 'User doesn\'t have this TODO!' });
  }
